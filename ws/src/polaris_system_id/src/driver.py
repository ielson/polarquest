...
def main():
    ...
    pub = rospy.Publisher("/car_like/ackermann_cmd", AckermannDriveStamped, queue_size=1)
        ...
        while not rospy.is_shutdown() and t < total_time:
            ...
            if seg != last_seg:
                if seg == 0:
                    ...
                    msg.drive.speed = clamp(fixed_speed, min_speed, speed_max)
                    msg.drive.steering_angle = 0.0
                    rospy.loginfo("[seg=%d] PRE  | speed=%.2f | delta=%.3f",
                                  seg, msg.drive.speed, msg.drive.steering_angle)
                elif seg == 1:
                    # direction jstep, maintaining fixed speed
                    msg.drive.speed = clamp(fixed_speed, min_speed, speed_max)
                    msg.drive.steering_angle = clamp(delta_step, -steer_max, steer_max)
                    ...
                    msg.drive.speed = clamp(new_speed, min_speed, speed_max)
                    msg.drive.steering_angle = clamp(new_steer, -steer_max, steer_max)
            ...
            if int(t * rate_hz) % rate_hz == 0:
                rospy.loginfo("t=%.1fs | speed=%.2f | delta=%.3f", t, msg.drive.speed, msg.drive.steering_angle)
